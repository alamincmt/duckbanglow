...
public class HotelAddActivity extends AppCompatActivity  implements
        ...
        btn_sub.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (propertyName.length()>1 && propertyDes.length()>1 && propertyPhone.length()>1 && propertyAddress.length()>1 && propertyBedRoom.length()>1 && propertyBathRoom.length()>1 && propertyArea.length()>1 && shortAddress.length()>1){
                    addMap.put("id",id);
                    addMap.put("propertyName",propertyName);
                    addMap.put("propertyDes",propertyDes);
                    addMap.put("propertyPhone",propertyPhone);
                    addMap.put("propertyAddress",propertyAddress);
                    addMap.put("propertyBedRoom",propertyBedRoom);
                    addMap.put("propertyBathRoom",propertyBathRoom);
                    addMap.put("propertyArea",propertyArea);
                    addMap.put("propertyPrice",propertyPrice);
                    ...
                    databaseReference.child(id).setValue(addMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                Toast.makeText(HotelAddActivity.this, propertyName+" added Successfully", Toast.LENGTH_SHORT).show();
                                propertyName = edt_Purpose_name.getText().toString().trim();
                                propertyDes = edt_Purpose_desc.getText().toString().trim();
                                propertyPhone = edt_Purpose_phone.getText().toString().trim();
                                propertyAddress = edt_Purpose_address.getText().toString().trim();
                                ...
                                Intent intent  = new Intent(HotelAddActivity.this,AdminDashboardActivity.class);
                        ...
                        public void onCanceled() {
                            Toast.makeText(HotelAddActivity.this, "Cancelled.", Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(HotelAddActivity.this, "Error "+e.getMessage(), Toast.LENGTH_SHORT).show();
